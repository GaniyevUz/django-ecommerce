...
from django.db.models import Model, OneToOneField, CASCADE, CharField, EmailField, FloatField, BooleanField, ForeignKey, \
    SET_NULL, DateTimeField, IntegerField, ImageField, DecimalField
...
class Product(Model):
    ...
    image = ImageField(upload_to='products/', default='images/placeholder.png')
    ...
    def image_url(self):
        try:
            ...
            url = 'https://stepswithcode.s3-us-west-2.amazonaws.com/m1-prt7/2+placeholder.png'
        return url
...
class Order(Model):
    ...
    def shipping(self):
        ...
        for orderitem in self.orderitem_set.all():
            shipping = True if not orderitem.product.digital else False
        return shipping
